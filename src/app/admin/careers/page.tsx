...
export default function AdminCareers() {
  ...
  const fetchCareers = async () => {
    try {
      ...
      const data = res?.data?.careers || res?.data || [];
  ...
  const toggleActive = async (id: string, current: boolean) => {
    try {
      await api.put(`/careers/${id}`, { isActive: !current });
      setCareers((prev) =>
  ...
  return (
    <div>
      ...
      <div className="admin-table">
        ...
        {careers.map((career) => (
          <div key={career._id} className="table-row">
            <span style={{ fontWeight: 600 }}>{career.title}</span>
            <span>{career.department || "—"}</span>
            <span>{career.location || "—"}</span>
            <span>
              <span className="type-badge">{career.type}</span>
            </span>
            <span>{career.applications || 0}</span>
            <span>
              <button
                onClick={() => toggleActive(career._id, career.isActive)}
                className={`toggle-btn ${career.isActive ? "active" : ""}`}
              >
            ...
            <span className="action-btns">
              ...
              <button onClick={() => handleDelete(career._id, career.title)} className="icon-btn danger" title="Delete">
      ...
      <style jsx>{`
        ...
        .table-header {
          display: grid;
          grid-template-columns: 1.5fr 1fr 1fr 0.8fr 0.9fr 0.7fr 0.9fr;
          padding: 14px 20px;
          background: var(--iv2);
          font-size: 11px;
          font-weight: 700;
        ...
        @media (max-width: 900px) {
          .table-header, .table-row {
            grid-template-columns: 1.5fr 1fr 0.8fr 0.7fr 0.9fr;
          }
