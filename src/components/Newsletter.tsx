        ...
        <div className="nl-inner" style={{ position: "relative", zIndex: 2, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "center" }}>
          ...
          <div className="rv-r">
            <div style={{ background: "rgba(249,246,240,.07)", backdropFilter: "blur(20px)", border: "1px solid rgba(249,246,240,.15)", borderRadius: "var(--r-xl)", padding: 40 }}>
              ...
              {[
                { label: "Your Name", type: "text", ph: "E.g. Rahul Gupta" },
                { label: "Email Address", type: "email", ph: "you@example.com" },
                { label: "Favourite Destination", type: "text", ph: "E.g. Bali, Maldives, Japan…" },
              ].map((f, i) => (
                <div key={i} style={{ marginBottom: 18, display: "flex", flexDirection: "column", gap: 7 }}>
                  <label className="syne" style={{ fontSize: 10, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", color: "rgba(249,246,240,.45)" }}>{f.label}</label>
                  <input type={f.type} placeholder={f.ph} style={{ padding: "13px 16px", background: "rgba(249,246,240,.08)", border: "1px solid rgba(249,246,240,.15)", borderRadius: 10, color: "#fff", fontFamily: "var(--font-inter),'Inter',sans-serif", fontSize: 14, outline: "none", transition: "border-color .2s" }} />
                </div>
              ))}
              <button className="syne" style={{ width: "100%", padding: 15, background: "var(--cu)", border: "none", borderRadius: 10, color: "#fff", fontSize: 14, fontWeight: 700, letterSpacing: 0.5, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, transition: "var(--tr)", boxShadow: "0 8px 28px rgba(0,174,204,.4)", marginTop: 4 }}>
                <span className="material-symbols-rounded">send</span>Subscribe &amp; Save Big
              </button>
